...
```
src/
  main.cpp          — entry point, calls parseArguments() then processFramesConcurrently()
  argparse.cpp/hpp  — CLI argument parsing (short and long flags)
  video.cpp/hpp     — Sierra dithering algorithm and frame processing
  concurrency.cpp/hpp — producer-consumer threading pipeline
CMakeLists.txt      — build config (C++23, OpenCV linkage)
```
...
- `findClosestColor()` does binary quantization at threshold 127.5
